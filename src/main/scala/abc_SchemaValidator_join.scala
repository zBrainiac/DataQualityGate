...
object abc_SchemaValidator_join {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dataset = spark.read
      ...
      .csv(input_path + input_filename).toDF()
    ...
    val lookup_star_rating = spark.read
      .option("header", value = true)
      .option("delimiter", ",")
      .option("inferSchema", "true")
      .csv(input_path + "lookup_star_rating,csv").toDF()

    lookup_star_rating.printSchema()

    println(s"\nlookup dataset:")
    lookup_star_rating.show(10)


    val enriched_dataset = dataset.join(lookup_star_rating,dataset("star_rating") === lookup_star_rating("star_rating")).select(dataset("*"),lookup_star_rating("description"))
      .toDF

    enriched_dataset.printSchema()
    println(s"\nenriched dataset:")
    enriched_dataset.show()
    ...
    val result = RowLevelSchemaValidator.validate(enriched_dataset, schema)

    println(s"\nnumValidRows: " + result.numValidRows)
    println(s"\nnumInvalidRows: " + result.numInvalidRows)

    println(s"\nInvalid Rows:")
    result.invalidRows.show(120, truncate = true)
